...
void SerialDebugWidget::initSerialPort(){
    serialPort = new QSerialPort(this);
    connect(serialPort, SIGNAL(readyRead()), this, SLOT(manual_serialPortReadyRead()));
}

void SerialDebugWidget::scanAndAddSerialPorts(){
...
void SerialDebugWidget::initStatusBar() {
    // 初始化发送、接收计数为0
    sendNum = 0;
    recvNum = 0;
    ...
    lblPortState->setStyleSheet("color:red"); // 设置端口状态标签颜色
    // 设置标签最小尺寸
    ...
    // 将标签添加到状态栏（从右往左）
    statusBar->addPermanentWidget(lblPortState);
    statusBar->addPermanentWidget(lblSendNum);
    statusBar->addPermanentWidget(lblRecvNum);

}


void SerialDebugWidget::initTimer() {
...
void SerialDebugWidget::on_serialopenbutton_clicked()
{
    //串口变量
...
void SerialDebugWidget::on_serialdetectbutton_clicked()
{
    scanAndAddSerialPorts();
}

void SerialDebugWidget::on_clearbitsButton_clicked()
{
    ui->recvEdit->clear();
    // 清除发送、接收字节计数
    sendNum = 0;
...
void SerialDebugWidget::on_TimeSend_checkbox_stateChanged(int arg1)
{
    // 获取复选框状态，未选为0，选中为2
    if(arg1 == 0){
        timSend->stop();
...
void SerialDebugWidget::on_sendbutton_clicked()
{
    QByteArray array;
...
void SerialDebugWidget::setNumOnLabel(QLabel *lbl, QString strS, long num)
{
    // 标签显示
    QString strN;
    strN.sprintf("%ld", num);
...
void SerialDebugWidget::on_clrsendbutton_clicked()
{
    ui->sendEdit->clear();
    // 清除发送字节计数
    sendNum = 0;
...
void SerialDebugWidget::manual_serialPortReadyRead()
{
